...
class CircularDoublyLinkedList:
    ...
    def append(self, value):
        """
    Purpose:
    Insert a new node at the **end** of the Circular Doubly Linked List (CDLL).
    ...
    Append(40):
        Step 1: new_node = [40]
        Step 2: new_node.prev = tail   (40.prev → 30)
        Step 3: new_node.next = head   (40.next → 10)
        Step 4: tail.next = new_node   (30.next → 40)
        Step 5: head.prev = new_node   (10.prev → 40)
        Step 6: tail = new_node        (tail → 40)

    After:
        [10] ◀──▶ [20] ◀──▶ [30] ◀──▶ [40]
         ↑head                         ↑tail
        (circular links: head.prev=40, tail.next=10)

    🔗 Pointer changes:
    - new_node.prev → old tail (30)
    - new_node.next → head (10)
    - old tail.next → new_node (40)
    - old head.prev → new_node (40)
    - tail → new_node (40)
